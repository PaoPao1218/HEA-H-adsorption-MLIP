...
- `hea_h_adsorption.csv` — per-site H adsorption energies (2500 rows), incl. `E_ads` and `dG_H ≈ E_ads + 0.24 eV`.
- `hea_formation_enthalpy.csv` — per-composition formation enthalpy (25 rows).
- `hea_pure_reference.csv` — per-element `E_ground` / `E_fcc` / `ΔE_struct` (from `hea_ref_energies.py`), used for the strict mixing-enthalpy (Ω) convention.
